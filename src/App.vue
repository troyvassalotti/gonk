...
import { getHonk, soundLibrary } from "./lib/utils";
...
<template>
  ...
  <aside class="what">
    <h2>What is Gonk?</h2>
    <p>
      Gonk is a platform for creating and sending <em>honks</em> to your
      friends. Honks are composed of a sound and a note. Create your honk by
      selecting a sound from our library and writing a personalized note for
      your recipient. You can then copy or share a URL to send along.
    </p>
  </aside>
  <div class="context">
    <SendHonk :sounds="soundLibrary" />
    <ReceiveHonk
      :honk="store.state.availableHonk"
      class="honkAvailable"
      id="receivedHonk"
    />
  </div>
...
@font-face {
  ...
  font-display: swap;
  src: url("/fonts/Recursive_VF_1.084--subset_range_english_basic.woff2")
    format("woff2");
...
.header,
.what {
  margin-block-end: clamp(2rem, 12.5vh, 5rem);
